<x-app-layout>
    ...
    <x-slot name="slot1">
        <x-breadcrumb.title title='Devolución de Inventario' />
        <x-breadcrumb.breadcrumb>

            <x-breadcrumb.breadcrumb2 name='Devolución de Inventario' />
        </x-breadcrumb.breadcrumb>
    </x-slot>
        ...
        <div class="mx-auto p-4" x-data="pos()">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3">
                ...
                <div class="p-2 text-gris-20 col-span-1">
                    <div class="mb-4  bg-gris-80 shadow-xl rounded-lg p-4">
                        <h3 class="mb-3">Seleccione la Tienda</h3>
                        <x-select x-model="store" @change="handleStoreChange()">
                            <option value="" disabled selected>Seleccione tienda</option>
                            @foreach ($stores as $store)
                                <option value="{{ $store->id }}">{{ $store->name }}</option>
                            @endforeach
                        </x-select>
                        <x-label class="my-2">Observaciones</x-label>
                        <x-input-textarea placeholder="Contenido" name="observation" col="4"
                        x-model="observationText"
                        ></x-imput-textarea>
                    </div>
                    <div class="mb-4  bg-gris-80 shadow-xl rounded-lg p-4">
                        ...
                        <div class="flex justify-between">
                            <button @click="checkout(); loading = true;" :disabled="cart.length === 0 || loading || store === ''"
                                :class="{
                                'bg-corp-50 hover:bg-corp-70 cursor-pointer': !loading && cart.length > 0 && store > 0,
                                'bg-corp-90 cursor-not-allowed': cart.length === 0 || loading  || store === ''
                            }"
                        ...
                        <x-dashboard.modal-js maxWidth="sm">
                            <x-slot name="title">
                                <h3 class="text-center">¡Devolución registrada con éxito!</h3>
                            </x-slot>
                            <x-slot name="content">
                                <x-elements.success scale="0.75"></x-elements.success>
                                <div class="text-center mt-3 ">
                                    <p>La Devolución <strong x-text="' # '+order "></strong> se ha registrado correctamente.</p>
    ...
    cart: [], client: {}, validations:{}, order:'', totalmodal:'', productosDiponibles:[],
    total: 0, store: '', observationText:'',
    ...
    handleStoreChange(){
        this.resetCart();
        if(this.store > 0) {
axios.get('/api/stores/'+this.store+'/skus') // Reemplaza '/api/datos' con tu URL
                    .then(response => {
    ...
    axios.post('{{ route('return.store') }}',datos) // Reemplaza '/api/datos' con tu URL
                    .then(response => {
                        this.order=response.data.return_id;
