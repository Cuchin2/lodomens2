<div>
    ...
    });" class="pb-4 sm:w-[442px] md:w-[711px] lg:w-[962px] xl:w-[1115px] mx-auto" @out.window="if(window.innerWidth < 768) { open = false; }">
        {{-- MENU DE FILTROS --}}
        <div class="fixed w-full sm:w-[442px] md:w-[711px] lg:w-[962px] xl:w-[1115px] bg-black z-20 left-1/2 transform -translate-x-1/2 md:top-[159px] px-3">
            <div class="flex py-3 mx-auto items-center">
                <div class="!w-[40.5px] h-[34px] bg-gris-90 rounded p-1 cursor-pointer flex items-center mr-2" @click="open=!open;" >
                ...
                <div class="hidden md:block w-fit mx-[50px] whitespace-nowrap">
                    <p>{{-- 107 --}} {{ $this->products->count() }} {{ $this->products->count() == 1 ? 'Resultado' :
                        'Resultados' }}</p>
                </div>
                <div class="w-full">
                    <div class="relative">
                    ...
                    <ul class="md:fixed bg-gris-90 md:w-[148px] lg:w-[205px]  rounded-[3px] h-fit md:max-h-full overflow-hidden xl:px-3"
                        ...
                        <li class="p-2 ">
                            ...
                            <div class="flex flex-wrap gap-1">
                                ...
                                @endif
                                @if($bra)
                                <x-web.special.filter wire:click="brandized('','')"  @click="if(window.innerWidth < 768) { open = false; }">{{ $bra}}</x-web.special.filter>
                                @endif
                                @if($gam)
                                <x-web.special.filter wire:click="colorized('','')"  @click="if(window.innerWidth < 768) { open = false; }">{{ $gam}}</x-web.special.filter>
                                @endif
                                @if($rating)
                                <x-web.special.filter wire:click="$set('rating', '')"  @click="if(window.innerWidth < 768) { open = false; }">{{ $rating == 1 ? '1 estrella' :
                                    $rating.' estrellas' }}</x-web.special.filter>
                                @endif
                        ...
                        <li class=" p-2" >
                            <div class="text-gris-10  cursor-pointer">
                                <p1 class="hover:text-white text-gris-10 flex items-center" @click="menu1 = (menu1 === 1) ? 0 : 1" >Categorías
                                ...
                                <ul x-show="menu1===1" x-collapse x-cloak
                                    ...
                                    <p2 wire:click="categorized('{{ $category->name }}','{{ $category->id }}')" @click="if(window.innerWidth < 768) { open = false; } else { menu1 = 0;}"
                        ...
                        <li class=" p-2" >
                            <div class="text-gris-10  cursor-pointer">
                                <p1 class="hover:text-white text-gris-10 flex items-center" @click="menu1 = (menu1 === 2) ? 0 : 2">Marcas
                                    <x-icons.chevron-right height="10px" width="10px" grosor="1" class="ml-auto"
                                ...
                                <ul x-show="menu1===2" x-collapse x-cloak class="w-fit text-[12px] ml-2 mt-2" >
                                    ...
                                    <p2 wire:click="brandized('{{ $brand->name }}','{{ $brand->id }}')"
                                        class="hover:text-white" @click="if(window.innerWidth < 768) { open = false; } else { menu1 = 0;}">{{ $brand->name }}</p2>
                                    @endforeach
                                </ul>
                            </div>
                        </li>
                            ...
                            <div class="text-gris-10  cursor-pointer">
                                ...
                                <ul x-show="menu1===3" x-collapse x-cloak class="flex flex-wrap mt-2">
                                    ...
                                    <img src="{{ asset('storage/'.$gama->images->url) }}"
                                        wire:click="colorized('{{ $gama->name }}','{{ $gama->id }}')" @click="if(window.innerWidth < 768) { open = false; } else { menu1 = 0;}"
                                        class="rounded-full mx-auto w-4 h-4" alt="">
                                    @else
                                    <div class="rounded-full w-4 h-4 mx-auto "
                                        wire:click="colorized('{{ $gama->name }}','{{ $gama->id }}')" @click="if(window.innerWidth < 768) { open = false; } else { menu1 = 0;}"
                                        style="background:{{ $gama->hex }}"></div>
                                    @endif
                                    @endforeach
                                </ul>
                            </div>
                ...
                <div wire:loading.remove wire:target="rate,categorized,brandized,colorized,clean,search,typerized" :class="{
                    ...
                    <div class="px-3 mx-auto relative my-[8px] items-center w-fit" x-data="{icon:false}"
                        ...
                        <a
                            ...
                            return (object) ['name' => $color->name, 'hex' => $color->hex, 'id' => $color->id, 'url' =>
                            $url];
                            }); $imagenes = [];
                            ...
                            $sku = \App\Models\Sku::where(['color_id' => $firstImage[$key0]->color_id, 'product_id' =>
                            $product->id])->first();
                            ...
                            $sku = \App\Models\Sku::where(['color_id' => $colorSelect[0]->id, 'product_id' =>
                            $product->id])->first();
                            if(session('location') !== 'PE')
                            { $sku->sell_price = $sku->usd; }
                            }
                        ...
                        <div class="m-2 leading-[1.2]" x-show="sort===3" x-cloak>
                            ...
                            <p class="text-[18px] md:text-[22px]"> {{ $sku->sell_price == 0 ? '' :
                                (session('currency').$sku->sell_price ?? '')}}</p>
                        </div>
