...
class TransferTable extends Component
{
    ...
    public function SendNote($name,$id,$salesperson_id){
        ...
        $note = TransferNote::where('transfer_id',$id)->first() ?? false;
        if($note) {
    ...
    {
        return view('livewire.transfer-table',[
            'sales' => Transfer::search($this->search)
