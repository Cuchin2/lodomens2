    ...
    @if (auth()->check())
        ...
        <li class="flex justify-center space-x-4">
            ...
            <div class="w-full" x-data="{ count: 0 }" x-init="count = $refs.countme.value.length">
                <textarea wire:model="comment" type="text" class="w-full border-gray-300 dark:border-gray-700 dark:bg-black dark:text-gris-30 focus:border-gris-50 dark:focus:border-gris-50 focus:ring-gris-50 dark:focus:ring-gris-50 rounded-lg shadow-sm text-[12px]" placeholder="Dejanos un comentario"
                    rows="3" maxlength="225" x-ref="countme" x-on:keyup="count = $refs.countme.value.length">{{ $comment }}</textarea>
                ...
                <button wire:click="save" type="button"
                    class=" mt-3 float-right bg-corp-50 rounded-[3px]"
        ...
        @foreach ($co as $com)
            <li class="w-full flex  px-0 space-x-5" x-data="{ abc: false }">
                ...
                <div class="w-full">
                    ...
                    <div class="item__date">- {{ Carbon\Carbon::parse($com->created_at)->isoformat('DD MMM YYYY, h:mm a') }}
                    </div>
                </div>
                    ...
                    <p class="my-0 ml-5 p-0 cursor-pointer hover:text-corp-30"
                        @if (auth()->check()) @click="abc = !abc" @endif>Responder</p>
                </div>
                @if (auth()->check())
                    <ul class="py-4" x-show="abc">
                        <li class="flex justify-center space-x-4">
                            ...
                            <div class="w-full" x-data="{ count: 0 }" x-init="count = $refs.countme.value.length">
                                <textarea wire:model="comment2" type="text" class="w-full border-gray-300 dark:border-gray-700 dark:bg-black dark:text-gris-30 focus:border-gris-50 dark:focus:border-gris-50 focus:ring-gris-50 dark:focus:ring-gris-50 rounded-lg shadow-sm text-[12px]" placeholder="Dejanos un comentario"
                                    rows="3" maxlength="225" x-ref="countme" x-on:keyup="count = $refs.countme.value.length">{{ $comment }}</textarea>

                                <div class="d-inline-flex ml-2">
                                    <template x-if="count < 20">
                                        <small id="helpId" class="form-text text-muted text-corp-30"
                                            x-html="count"> /</small>
